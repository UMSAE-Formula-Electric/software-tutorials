	...
	/**
	  ...
	  * @retval int
	  */
	//TODO: queueID,  msgPointer, queuePriority and timeout
	int main(void)
	{
	...
	void StartQueueAppendTask(void *argument)
	{
	  ...
	  for(;;)
	  {
		...
		osMessageQueuePut(coolQueueHandle, &dataSend, 0, 0);
	...
	*/
	...
	void StartQueueRemoveTask(void *argument)
	{
	  ...
	  for(;;)
	  {
		if(osMessageQueueGet(coolQueueHandle, &dataReceived, NULL, 99999) == 0)
		{
			snprintf(new_char,sizeof(new_char),"%d\r\n",dataReceived);
			HAL_UART_Transmit(&huart2, (uint8_t*)new_char, strlen(new_char), 99999);
		}
		osDelay(pdMS_TO_TICKS(100));
	  }
