...
void StartQueueRemoveTask(void *argument)
{
  ...
  for(;;)
  {
      osMessageQueueGet(coolQueueHandle, &dataReceived, NULL, osWaitForever);
      snprintf(new_char, sizeof(new_char), "%d\r\n", dataReceived);
      HAL_UART_Transmit(&huart2, (uint8_t*)new_char, strlen(new_char), HAL_MAX_DELAY);
      osDelay(pdMS_TO_TICKS(100));
  }
